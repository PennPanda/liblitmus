...
do
	...
	do
		for ((num_tasks=${num_tasks_min}; num_tasks<=${num_tasks_max}; num_tasks+=${num_tasks_step}));do
            ...
            echo "python ./analyze_xentrace_csv2result.py ${input_csv_file} ${output_do_sched} ${output_cxt_switch} ${output_cxt_saved}"
            python ./analyze_xentrace_csv2result.py ${input_csv_file} ${output_do_sched} ${output_cxt_switch} ${output_cxt_saved}
		done
	done
done
done
