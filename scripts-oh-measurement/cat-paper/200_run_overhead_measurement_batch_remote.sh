	...
	do
		for ((num_tasks=${num_tasks_min}; num_tasks<=${num_tasks_max}; num_tasks+=${num_tasks_step}));do
            for ((dom_id=${dom_id_min}; dom_id<=${dom_id_max}; dom_id+=${dom_id_step})); do
                echo "ssh root@dom${dom_id} \"cd ${SCRIPTS_ROOT}; ${SCRIPTS_ROOT}/run_overhead_measurement.sh ${case} ${DUR} ${WAIT} ${alg} ${type} ${num_tasks} ${RTTASK} ${dom_id} ${ENV}\" &"
                ssh root@dom${dom_id} "cd ${SCRIPTS_ROOT}; ${SCRIPTS_ROOT}/run_overhead_measurement.sh ${case} ${DUR} ${WAIT} ${alg} ${type} ${num_tasks} ${RTTASK} ${dom_id} ${ENV}" &
            done
            ...
            xentrace -D -e 0x0001f000 -T ${DUR} ${xen_trace_file}
            echo "sleep ${DUR}"
            sleep ${DUR}
            sleep 20
		done
