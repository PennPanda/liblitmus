...
static int job(int wss, int shuffle, double exec_time, double program_end)
{
	if (wctime() > program_end)
		...
		if (strict_wcet && job_no > 1) {
			double nsec = (finish.tv_sec - start.tv_sec) + 
				(finish.tv_nsec - start.tv_nsec)/1.0e9;
			
			printf ("STRICT: MEASURE %.9f - %.9f = %.9f\n", exec_time, nsec, exec_time - nsec);

			if (nsec > exec_time) {
				printf("STRICT: FAILURE wcet target %.9f current %.9f\n",
					exec_time, nsec);
				return -1;
			}
...
#define OPTSTR "p:c:C:weq:r:l:S:U:f:t"
int main(int argc, char** argv)
{
	...
	while ((job_ret=job(wss, shuffle, wcet_ms * 0.001, start + duration)) == 1)
	{
		job_no++;
	};
	...
	printf("ca_spin_hard finished.\n");

	if (job_ret < 0) {
		printf("failed to complete job: %d jobs executed\n", job_no);
		return -job_ret;
	}
