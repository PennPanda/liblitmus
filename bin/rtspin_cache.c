...
char* Array;
long  Array_size = 480 * 1024; //480kb
long  Iteration = 10;
...
static void usage(char *error) {
	...
	fprintf(stderr,
		...
		"	       [-S array size] [-I number of loops]\n"
		"\n"
...
static int init_array(long size)
{
	int i;
	Array_size = size;
	...
	printf("init_array finished.\n");
	return 0;
}

#define OPTSTR "p:c:C:wlveo:f:s:q:X:L:Q:S:I:"
int main(int argc, char** argv)
{
	...
	if (Array_size <= 0)
	{
		printf("Must specify -S array_size_kb\n");
		exit(1);
	}
	init_array(Array_size);
	if (iteration <= 0)
	{
		printf("Must specify -I iteration\n");
		exit(1);
	}
	Iteration = iteration;
