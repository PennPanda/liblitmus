...
static int cacheline_walk(cacheline_t *mem, int wss) {
	...
	for (i = 0; i < numlines; i++) {
		//only line[0] is used to index the next item to walk
		//write to line[1] to make cache line dirty
		if (INTS_IN_CACHELINE > 1)
			arena[next].line[1] = next + 1;
		else
			fprintf(stderr, "Only one int in a cache line\n");
